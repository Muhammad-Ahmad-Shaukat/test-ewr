...
export default function Team() {
  ...
  const handleMouseEnter = () => {
    ...
    console.log(
      `🖱️ Hover started at ${startTime} - starting animation immediately`
    );
    setIsHovered(true);
    ...
    hoverTimeoutRef.current = setTimeout(() => {
      ...
      setTimeout(() => {
        ...
        console.log(
          `✅ Transition complete at ${transitionEndTime} - switching to next set`
        );
        ...
        intervalRef.current = setInterval(() => {
          ...
          console.log(
            `🔄 Starting image transition at ${regularTransitionStartTime}...`
          );
          setIsTransitioning(true);
          setIsNewImage(true);
          ...
          setTimeout(() => {
            ...
            console.log(
              `✅ Transition complete at ${regularTransitionEndTime} - switching to next set`
            );
            // First, update display states used for rendering
            setDisplayPatternA((prev) => !prev);
            setDisplaySet((prev) => (prev + 1) % Math.ceil(team.length / 5));
  ...
  const nextImages = getSequentialImages(
    (displaySet + 1) % Math.ceil(team.length / 5)
  );
  ...
  return (
    <div
      className="team w-full max-w-[400px] rounded-2xl shadow-lg lg:px-[29px] px-[15px] md:pt-[29px] pt-[25px] lg:pb-10 pb-[25px]"
    ...
    >
      ...
      <p className="text-[16px] leading-[145%] text-[#131316] md:mb-[91px] mb-[50px] font-normal">
        The people who bring our culture, speed, and expertise to life.
      </p>
