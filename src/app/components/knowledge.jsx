...
const ExpertKnowledge = () => {
    ...
    const animate = () => {
      setParticles(prevParticles => 
        prevParticles.map(particle => {
          ...
          const animationProgress = Math.min((elapsed - particle.delay) / 0.8, 1); // 0.8 second animation
          ...
          const getCurvedPosition = (progress, personIndex) => {
            ...
            if (personIndex === 0) {
              ...
              if (t < 0.15) {
                ...
                return {
                  x: mt3 * p1.x + 3 * mt2 * localT * p2.x + 3 * mt * t2 * p3.x + t3 * p4.x,
                  y: mt3 * p1.y + 3 * mt2 * localT * p2.y + 3 * mt * t2 * p3.y + t3 * p4.y
                };
              } else if (t < 0.7) {
          ...
          const pos = getCurvedPosition(animationProgress, particle.personIndex);
  ...
  return (
    <div className=" w-[100%] max-w-[400px] bg-white rounded-[16px] shadow-lg flex items-center justify-center md:px-[29px] px-[20px] md:pt-[29px] pt-[25px] pb-[0px]">
      <div className="max-w-4xl w-full">
        {/* Header */}
        <div className="text-center md:mb-[20px] mb-[40px]">
          ...
          <p className="text-[16px] leading-[145%] text-[#131316] text-left font-normal">
            Engineers with the expertise to deliver dependable, high-performance solutions
          </p>
        </div>
          ...
          <div className="pointer-events-none relative flex-auto select-none" style={{minHeight: '10.25rem'}} aria-hidden="true">
            ...
            <div className="relative flex h-full flex-col items-center justify-center">
              <div className="absolute -z-10 mt-[calc(-108/16*1rem)] blur-[1px]">
                <div className="absolute left-1/2 top-1/2 ml-[calc(-216/2/16*1rem)] mt-[calc(-216/2/16*1rem)] size-[calc(216/16*1rem)] rounded-full border border-gray-400 opacity-15" style={{transform: 'scale(1)'}}></div>
                <div className="absolute left-1/2 top-1/2 ml-[calc(-280/2/16*1rem)] mt-[calc(-280/2/16*1rem)] size-[calc(280/16*1rem)] rounded-full border border-gray-400 opacity-[0.125]" style={{transform: 'scale(1)'}}></div>
                <div className="absolute left-1/2 top-1/2 ml-[calc(-344/2/16*1rem)] mt-[calc(-344/2/16*1rem)] size-[calc(344/16*1rem)] rounded-full border border-gray-400 opacity-10" style={{transform: 'scale(1)'}}></div>
                <div className="absolute left-1/2 top-1/2 ml-[calc(-408/2/16*1rem)] mt-[calc(-408/2/16*1rem)] size-[calc(408/16*1rem)] rounded-full border border-gray-400 opacity-[0.075]" style={{transform: 'scale(1)'}}></div>
              </div>
              ...
              <div className="relative aspect-[113/55] w-32">
                <div>
                  ...
                  <svg viewBox="0 0 113 55" fill="none" aria-hidden="true" className="absolute inset-0 size-full stroke-gray-300" strokeWidth="1">
                    <path d="M56.5078 0V25M0.507812 0V8C0.507812 16.837 7.67081 24 16.5078 24H40.5078C49.3448 24 56.5078 31.163 56.5078 40V55M112.508 0V8C112.508 16.837 105.345 24 96.5078 24H72.5078C66.5858 24 61.4148 27.218 58.6478 32" stroke="#131316" strokeOpacity="0.1"/>
                  </svg>
              ...
              <div className="relative mt-px flex items-center gap-1.5 rounded-lg bg-white py-1 pl-1.5 pr-2 text-2xs font-medium text-gray-950 shadow-[0_1px_1px_rgba(0,0,0,0.05),0_4px_6px_rgba(34,42,53,0.04),0_24px_68px_rgba(47,48,55,0.05),0_2px_3px_rgba(0,0,0,0.04)] ring-1 ring-gray-950/5">
                ...
                Trusted Expertise
                <div className="absolute -bottom-1.5 left-1/2 -z-10 -ml-10 h-6 w-20 transform-gpu rounded-[50%] bg-gradient-to-r from-purple-500 from-25% to-sky-300 to-75% blur-sm opacity-25"></div>
              </div>
            </div>
          </div>
        </div>
